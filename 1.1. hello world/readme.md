...
```
> [!WARNING]  
> На плате esp32s3 uno нет встроенного простого светодиода, поэтому для этого примера предлагается воспользоваться внешним светодиодом. Например модулем светофора, с которым будем работать позднее
