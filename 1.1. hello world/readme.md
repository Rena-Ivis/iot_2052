...
2. Устанавливаем EIM: Запустите Windows PowerShell -> В терминале PowerShell выполните команду: ```winget install Espressif.EIM```
3. Устанавливаем ESP-IDF: Запускаем ESP-IDF Installation Manager (EIM) -> New Installation -> Start Installation.
> [!TIP]
> Если на компьютере уже установлена какая-либо версия ESP-IDF, то дополнительно с New Installation будет отображаться элемент Manage Installations.
4. При первом запуске расширения ESP-IDF предлагается установить все необходимые компоненты. Предлагается несколько вариантов установки: Easy (самый простой, все настройки по умолчанию), Advanced (придётся указать ряд параметров самостоятельно) и Custom – полностью настраиваемая установка для опытных пользователей (можно выбрать путь размещения, версию ESP-IDF и даже установить сразу несколько версий),  Offline – установка без подключения к интернету (требуется архив, который можно получить и перенести с другого компьютера, подключенного к интернет), а вариант Load Configuration позволяет загрузить конфигурационный файл, созданных в процессе предыдущей установки, что позволяет ускорить и автоматизировать процесс установки.
5. После установки переходим к начальному экрану приложения EIM - Go to Dashboard. Тут можно аосмотреть установленные версии и кнопкой Install New Versions можно запустить процедуру установки других версий ESP-IDF, а для перехода к начальному экрану следует нажать на логотип Espressif Systems в левом верхнем углу.
6. Запускаем Visual Studio Code, открываем менеджер расширений, в строке поиска указываем esp-idf и для найденного расширения нажимаем кнопку Install.
Поскольку мы впервые устанавливаем расширение от поставщика Espressif Systems, то у нас спросят доверяем ли мы ему – соглашаемся, нажав на кнопку Trust Publisher & Install
После успешной установки расширения в боковой панели Visual Studio Code появится иконка расширения ESP-IDF
7. Переходим в расширение и в меню команд выбираем New Project Wizard -> ESP-IDF Templates -> sample_project(sample_project_cpp) -> Create project using template ... -> указываем имя проекта – например hello_world (пробелы, кириллицу и т.д. не используем!), проверяем, чтобы директория проекта не содержала не-ASCII символов, при необходимости меняем каталог. В поле Choose ESP-IDF Board выбираем esp32s3 -> Create project -> Open project
8. Раскроем дерево проекта, выберем файл main.c и посмотрим его содержимое - каркас пустой программы. Заменим текст файла main.c на следующий код программы:
```c
#include <stdio.h>
...
```
> [!TIP]
> Можно вместо этой программы использовать готовый шаблон на 7 шаге get started -> hello world
...
9. Ознакомимся с панелью инструментов ESP-IDF. В Visual Studio Code она располагается снизу.
![image](img/panel.png)
10. Запускаем процесс компиляции и сборки проекта. При успешной сборке на вкладке OUTPUT, которая открывается по умолчанию, будет выведено соответствующее сообщение и на некоторое время появится всплывающее окно с сообщением об успешном окончании операции сборки. Также при успешной сборке на вкладке TERMINAL отображается информация о распределении памяти микроконтроллера.
11. Убедиться в работоспособности оборудования и узнать, какой номер порта назначен плате, которую подключили к компьютеру, в ОС Windows можно запустив Диспетчер устройств. На основе этой информации нужно выбрать соответствующий порт в проекте и метод программирования (UART).

![image](img/win_dev_manager.png)

12. Программируем микроконтроллер – заливаем прошивку. Информацию об окончании процесса загрузки прошивки (Flash Done) можно увидеть на вкладке OUTPUT и во всплывающем окне. Переходим к монитору последовательного порта, к которому подключена плата – именно через него на вкладке TERMINAL можно наблюдать вывод программы.
...
## 1.1.2 Blink — мигаем светодиодом
...
#define LED GPIO_NUM_2 // указываем номер GPIO к которому подключен светодиод
...
```
> [!WARNING]  
> На плате esp32s3 uno нет встроенного простого светодиода, поэтому для этого примера предлагается воспользоваться внешним светодиодом. Например модулем светофора, с которым будем работать позднее (не забудьте указать нужный номер GPIO к которому подключен диод на плате)
