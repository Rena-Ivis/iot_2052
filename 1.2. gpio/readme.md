...
Для чтения регистров будем использовать макрос `REG_READ()`, которому нужно передать номер считываемого регистра.
