...
# Кейс: Контроль температуры и влажности на фармацевтическом складе
...
В рамках выполнения следующего заданий закрываются темы работы с таймерами, потоками и прерываниями

# Задача: Холодильная камера
>[!TIP]
>1. Необходимо разработать устройство – программно-аппаратную модель промышленной холодильной камеры объемом 20 л. и мощностью 70 Вт. Температура в камере поддерживается на заданном уровне за счет работы замкнутой системы управления с ПИД-регулятором.
>2. Требуемая температура охлаждения задаётся множеством предопределённых дискретных значений: +1 °C, +2 °C, +5 °C, +8 °C, переключаемых по нажатию кнопки «Temperature» на панели управления (или консоли).
>3. Температура снаружи холодильной камеры – комнатная. Она измеряется с помощью сенсора BME280(или другого доступного сенсера). Это значение должно учитываться в модели как теплообмен с внешней средой, особенно когда дверь камеры отрыта.
>4. Для моделирования открытия двери холодильной камеры использовать кнопку «Door», подключенную к устройству.
>5. Текущий уровень мощности холодильного агрегата отразить в виде яркости свечения светодиода, подключенного к устройству (при охлаждении 70 Вт – 100% яркость).
>6. Холодильник оснащён OLED-дисплеем, на котором при нажатии кнопки «Info» последовательно отображается температура внутри холодильника (моделируемое значение) и температура с датчика температуры окружающего воздуха. Спустя 5 с. после последнего нажатия на эту кнопку любые значения на дисплее исчезают. ```Упращенная версия: выводить в консоль.```
>7. Обработка внешних событий (нажатие кнопок) должна осуществляться по прерываниям.
>8. По последовательному каналу связи в консоль хост-компьютера, к которому подключено устройство, должен выводиться лог (с временными метками), включающий моменты открытия/закрытия дверцы, изменения мощности, изменения температуры внутри холодильника больше, чем на 1 °C.
> # Расширение бизнес-логики (необязательно, но приветствуется):
>
>При открытии дверцы («Door») более чем на 30 с. (моделирование забывчивости персонала), выведите в лог сообщение [ALERT] Нарушение режима эксплуатации! Это симулирует отправку аварийного уведомления управляющему.
